...
BATCH_SIZE = 4
LR = 1e-3
EPOCHS = 25           # 3-hour run
MAX_FILES = 3000      # Next 3000 files to extend previous training
...
class EDCDataset(Dataset):
    def __init__(self, feature_csv, edc_dir, max_files=None, downsample=1):
        ...
        if max_files is not None and max_files < len(self.df):
            # Take the next set of files (after previous 3000)
            self.df = self.df.iloc[3000:3000 + max_files].reset_index(drop=True)
...
class TCN_EDC(pl.LightningModule):
    ...
    def forward(self, x):
        x = self.fc(x)                        
        x = x.unsqueeze(-1).repeat(1, 1, self.edc_len)  
        x = self.tcn(x)
...
if __name__ == "__main__":
    ...
    # TensorBoard logger
    logger = TensorBoardLogger("tb_logs", name="tcn_edc2")
    ...
    # Save validation predictions without overwriting previous data
    x_val, y_val = next(iter(val_loader))
    y_hat = model(x_val)
    if os.path.exists("sample_predictions.npy"):
        old_preds = np.load("sample_predictions.npy")
        combined_preds = np.concatenate((old_preds, y_hat.detach().cpu().numpy()), axis=0)
    else:
        combined_preds = y_hat.detach().cpu().numpy()
    np.save("sample_predictions.npy", combined_preds)
    print(f"Sample predictions saved. Total predictions: {combined_preds.shape[0]}")
